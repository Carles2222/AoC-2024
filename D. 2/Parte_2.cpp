...
bool esSeguraConDampener(const vector<int>& informe) {
    if (esSegura(informe)) {
        return true;
    }

    for (size_t i = 0; i < informe.size(); ++i) {
        vector<int> copiaInforme = informe;
        copiaInforme.erase(copiaInforme.begin() + i); 

        if (esSegura(copiaInforme)) {
            return true; 
        }
    }
...
int main() {
    ...
    while (getline(archivoEntrada, linea)) {
        ...
        if (esSeguraConDampener(informe)) {
            cantidadSeguras++;
        }
    }
